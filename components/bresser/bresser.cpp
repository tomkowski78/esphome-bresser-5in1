...
//    Bresser() : PollingComponent(5000) { }
    ...
    void Bresser::setup() {
        ...
        if (state == RADIOLIB_ERR_NONE) {
            ...
            state = this->radio->setCrcFiltering(false);
            if (state != RADIOLIB_ERR_NONE) {
    ...
    //            while (true)  ;
            ...
            state = this->radio->setSyncWord(0xAA, 0x2D, 0, false);
            if (state != RADIOLIB_ERR_NONE) {
    ...
    void Bresser::update() {
        ...
        if (decode_ok) {
            if (weatherData.temp_ok){
                temperature_sensor_->publish_state(weatherData.temp_c);
...
//                wind_gust_sensor->publish_state(weatherData.wind_gust_meter_sec);
                wind_direction_sensor_->publish_state(weatherData.wind_direction_deg);
            }
    ...
    //
    void Bresser::genData(WeatherData_t *pOut)
    {
        pOut->sensor_id = 0xff;
        pOut->temp_c = 22.2f;
        pOut->humidity = 55;
    ...
    //
    uint16_t Bresser::lfsr_digest16(uint8_t const message[], unsigned bytes, uint16_t gen, uint16_t key)
    {
        uint16_t sum = 0;
    ...
    // From from rtl_433 project - https://github.com/merbanan/rtl_433/blob/master/src/util.c
    //
    int Bresser::add_bytes(uint8_t const message[], unsigned num_bytes)
    {
        int result = 0;
    ...
    */
    ...
    DecodeStatus Bresser::decodeBresser5In1Payload(uint8_t *msg, uint8_t msgSize, WeatherData_t *pOut) { 
    ...
    */
    DecodeStatus Bresser::decodeBresser6In1Payload(uint8_t *msg, uint8_t msgSize, WeatherData_t *pOut) {
    ...
    //
    uint8_t Bresser::windspeed_ms_to_bft(float ms)
    {
    if (ms < 5.5) {
        // 0..3 Bft
        if (ms < 0.9) {
    ...
    //
    bool Bresser::getWeatherdata(void)
    {
        uint8_t recvData[27];
        bool decode_ok = false;
    ...
    #ifdef _DEBUG_MODE_
    void Bresser::printRawdata(uint8_t *msg, uint8_t msgSize) {
        char s[120];
        uint8_t ii = 0;
        s[0] = 0;
